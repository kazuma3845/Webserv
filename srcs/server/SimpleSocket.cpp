...
{
	std::cout << "New SimpleSocket called" << std::endl;
}

SimpleSocket::SimpleSocket(int domain, int type, int protocol, int port, u_long interface_IP)
{
	...
	test_connection(this->_server_fd);
	...
	int opt = 1;
	this->_setsockopter = setsockopt(this->_server_fd, SOL_SOCKET, SO_REUSEADDR, (char *)&opt, sizeof(opt));
	test_connection(this->_setsockopter);

	// Bind socket to IP address and port
	this->_binder = bind(this->_server_fd, (struct sockaddr *)&this->_address, sizeof(this->_address));
	test_connection(this->_binder);

	// Set a listener ont the socket
	this->_listener = listen(this->_server_fd, 512);
	test_connection(this->_listener);

	std::cout << "SimpleSocket was called." << std::endl;

}

SimpleSocket::~SimpleSocket(void)
{
	std::cout << "SimpleSocket was destroyed." << std::endl;
}

SimpleSocket::SimpleSocket( const SimpleSocket& copy )
{
	std::cout << "SimpleSocket copy constructor called" << std::endl;
	*this = copy;
}

SimpleSocket& SimpleSocket::operator=( const SimpleSocket& ref )
{
	std::cout << "SimpleSocket assignment operator called" << std::endl;
	if ( this != &ref )
	{
		this->_server_fd = ref._server_fd;
...
void SimpleSocket::test_connection(int value)
{
	if (value < 0)
	{
...
void SimpleSocket::run_server(void)
{
	fd_set			read_fds;
	...
	while (1) {
		...
		FD_SET(this->_server_fd, &read_fds);
		...
		if (FD_ISSET(this->_server_fd, &read_fds))
		{
			addrlen = sizeof(this->_address);
			new_socket = accept(this->_server_fd, (struct sockaddr *)&this->_address, (socklen_t*)&addrlen);
		...
		for (int i = 0; i < MAX_CLIENTS; i++) {
			sd = this->_client_fds[i];
			if (sd != this->_server_fd && FD_ISSET(sd, &read_fds)) {
...
int SimpleSocket::get_server_fd(void)
{
	return this->_server_fd;
}

int SimpleSocket::get_binder(void)
{
	return this->_binder;
}

int SimpleSocket::get_listener(void)
{
	return this->_listener;
}

int SimpleSocket::get_setsockopter(void)
{
	return this->_setsockopter;
}

int SimpleSocket::get_activity_mon(void)
{
